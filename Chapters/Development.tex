...
which contains the database used by the Correlation Engine, a feature that finds files present in multiple cases to ingest data that can be queried by any case.
...
The authentication can be performed with either username or e-mail and a password, and extra authentication factors can be added such as \acrfull{otp} and \acrfull{u2f}.
...
Because even though the platform still wasn't fully developed, it already suffered from possible exploits that could break the experience for its users, like the upload of a 5GB file
